...
class QAT_trainer:
    def __init__(self, args):
        ...
        self.freeze_observer_epoch = args.freeze_observer_epoch
        self.freeze_bn_epoch = args.freeze_bn_epoch
    ...
    def prepare_model(self):
        if self.model_name == "vgg19":
            self.model = vgg19_quantized(num_class=self.num_class, dataset=self.type)
    ...
    def train(self, epoch):
        ...
        if epoch > self.freeze_observer_epoch:
    ...
    def test(self, epoch):
        self.model.eval()
        quantized_model = torch.quantization.convert(self.model.to('cpu'), inplace=False)
...
def main():
    ...
    parser.add_argument("--freeze-observer-epoch", default=5, type=int)
    parser.add_argument("--freeze-bn-epoch", default=4, type=int)
    ...
    args.result_path = "./" + args.result_dir + "/" + args.model_name + "_" + args.type + \
                       "/quantized_seed{}_lr{}_{}_epoch{}".format(args.seed, args.lr, lr_setting, args.epoch_num)

    seed_torch(args.seed)
    args.data_path = args.root
    trainer = QAT_trainer(args)
    trainer.benchmark()
